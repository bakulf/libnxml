...
#define __NXML_NAMECHARS                                                       \
  (__NXML_NAMESTARTCHARS || ch == '-' || ch == '.' ||                          \
   (ch >= '0' && ch <= '9') || ch == 0xb7 || (ch >= 0x0300 && ch <= 0x036f) || \
   (ch >= 0x203f && ch <= 0x2040))
...
int64_t __nxml_int_charset(int i, unsigned char *buffer, char *charset);

int __nxml_utf_detection(char *r_buffer, size_t r_size, char **buffer,
                         size_t *size, nxml_charset_t *);
