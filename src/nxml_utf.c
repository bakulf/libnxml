...
int64_t __nxml_utf8(unsigned char **buffer, size_t *size, int *byte) {
  int64_t ret;
  ...
  if ((c0 & 0xf8) == 0xf0 || *size < 5) {
    *byte = 4;
    ret = (((c0 & 0x7) << 18) | ((c1 & 0x3f) << 12) | ((c2 & 0x3f) << 6) |
           (c3 & 0x3f));
    return ret;
  }
  ...
  if ((c0 & 0xfc) == 0xf8) {
    *byte = 5;
    ret = (((c0 & 0x3) << 24) | ((c1 & 0x3f) << 18) | ((c2 & 0x3f) << 12) |
           ((c3 & 0x3f) << 6) | (c4 & 0x3f));
    return ret;
  }
...
static size_t __nxml_utf16to8(int le, unsigned char *buffer, size_t size,
...
static size_t __nxml_ucs4to8(int type, unsigned char *buffer, size_t size,
                             unsigned char **ret_buffer) {
  int64_t ch;
  int j = 0;
  int byte = 0;
  ...
  while (size > 0) {
    switch (type) {
    case 0: /* 1234 */
      ch = (*buffer << 18) + (*(buffer + 1) << 12) + (*(buffer + 2) << 6) +
           (*(buffer + 3));
      break;

    case 1: /* 4321 */
      ch = (*buffer) + (*(buffer + 1) << 6) + (*(buffer + 2) << 12) +
           (*(buffer + 3) << 18);
      break;

    case 2: /* 2143 */
      ch = ((*buffer) << 12) + (*(buffer + 1) << 18) + (*(buffer + 2)) +
           (*(buffer + 3) << 6);
      break;

    case 3: /* 3412 */
      ch = ((*buffer) << 6) + (*(buffer + 1)) + (*(buffer + 2) << 18) +
           (*(buffer + 3) << 12);
      break;
    }
...
int __nxml_utf_detection(char *r_buffer, size_t r_size, char **buffer,
  ...
  if (*r_buffer == 0x00 && *(r_buffer + 1) == 0x3c && *(r_buffer + 2) == 0x00 &&
      *(r_buffer + 3) == 0x3f) {
    (*size) = __nxml_utf16to8(1, (unsigned char *)r_buffer, r_size,
  ...
  if (*r_buffer == 0x3c && *(r_buffer + 1) == 0x00 && *(r_buffer + 2) == 0x3f &&
      *(r_buffer + 3) == 0x00) {
    (*size) = __nxml_utf16to8(0, (unsigned char *)r_buffer, r_size,
                              (unsigned char **)buffer);
  ...
  if (*r_buffer == 0x00 && *(r_buffer + 1) == 0x00 && *(r_buffer + 2) == 0x00 &&
      *(r_buffer + 3) == 0x3c) {
    (*size) = __nxml_ucs4to8(0, (unsigned char *)r_buffer, r_size,
                             (unsigned char **)buffer);
  ...
  if (*r_buffer == 0x3c && *(r_buffer + 1) == 0x00 && *(r_buffer + 2) == 0x00 &&
      *(r_buffer + 3) == 0x00) {
    (*size) = __nxml_ucs4to8(1, (unsigned char *)r_buffer, r_size,
                             (unsigned char **)buffer);
  ...
  if (*r_buffer == 0x00 && *(r_buffer + 1) == 0x00 && *(r_buffer + 2) == 0x3c &&
      *(r_buffer + 3) == 0x00) {
    (*size) = __nxml_ucs4to8(2, (unsigned char *)r_buffer, r_size,
                             (unsigned char **)buffer);
  ...
  if (*r_buffer == 0x00 && *(r_buffer + 1) == 0x3c && *(r_buffer + 2) == 0x00 &&
      *(r_buffer + 3) == 0x00) {
    (*size) = __nxml_ucs4to8(3, (unsigned char *)r_buffer, r_size,
                             (unsigned char **)buffer);
